...
void Serveur::errorSocket(QAbstractSocket::SocketError error)
{
	switch(error)
	{
		case QAbstractSocket::HostNotFoundError:
            affichage->append(tr("<em>ERROR : can't find Hydra server.</em>"));
			break;
...
{
    affichage->append("Connecting... to Hyrda IRC. Please wait.");

	sendData("USER "+pseudo+" localhost "+serveur+" :"+pseudo);
    sendData("NICK "+pseudo);
    affichage->append("Connected to Hydra IRC.");

}

void Serveur::joins()
{
    join("#Codename-Hydra.com");
}
